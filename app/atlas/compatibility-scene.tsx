...
export default function CompatibilityScene({atlas,state,onSelect,onProgress,onError}:Props) {
  ...
  useEffect(()=>{
    ...
    canvas.setAttribute("role","img");canvas.setAttribute("aria-label","Reduced-detail human anatomy compatibility view. Drag to rotate; use the view and zoom buttons, or select a structure.");
    ...
    el.appendChild(canvas);let disposed=false,frame=0,parts:LitePart[]=[],faces:Face[]=[],width=0,height=0,angle=0,tilt=0,viewKey="",drag:{x:number;y:number}|null=null;
    ...
    function draw(){
      ...
      if(viewKey!==key){angle=s.view==="back"?Math.PI:s.view==="side"?Math.PI/2:s.view==="three-quarter"?.38:0;tilt=0;viewKey=key;}
      ...
      ctx.fillStyle="#f1f3ec";ctx.fillRect(0,0,width,height);faces=[];if(!showing.length)return;
      const scale=Math.min((width-65)/Math.max(.025,right-left),(height-70)/Math.max(.025,top-bottom))*.92*Math.pow(1.2,s.zoom??0),cx=(left+right)/2,cy=(top+bottom)/2;
    ...
    const down=(e:PointerEvent)=>{tap.down(e.pointerId,e.clientX,e.clientY,7);drag={x:e.clientX,y:e.clientY};canvas.setPointerCapture(e.pointerId);};
    const move=(e:PointerEvent)=>{tap.move(e.pointerId,e.clientX,e.clientY);if(!drag)return;angle+=(e.clientX-drag.x)*.012;tilt=Math.max(-1,Math.min(1,tilt+(e.clientY-drag.y)*.008));drag={x:e.clientX,y:e.clientY};schedule();};
    const up=(e:PointerEvent)=>{drag=null;const valid=tap.up(e.pointerId,e.clientX,e.clientY);if(!valid)return;const r=canvas.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;for(let i=faces.length-1;i>=0;i--){const f=faces[i];if(f.alpha<.4)continue;const d=(f.y2-f.y3)*(f.x1-f.x3)+(f.x3-f.x2)*(f.y1-f.y3);if(Math.abs(d)<.01)continue;const a=((f.y2-f.y3)*(x-f.x3)+(f.x3-f.x2)*(y-f.y3))/d,b=((f.y3-f.y1)*(x-f.x3)+(f.x1-f.x3)*(y-f.y3))/d;if(a>=0&&b>=0&&a+b<=1){selection.current(f.id);break;}}};
    const cancel=(e:PointerEvent)=>{drag=null;tap.cancel(e.pointerId);};
